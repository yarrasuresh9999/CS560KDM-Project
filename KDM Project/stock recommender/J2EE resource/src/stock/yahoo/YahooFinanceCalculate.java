...
import stock.datamodel.CalStock;

public class YahooFinanceCalculate {
	...
	public static List<CustomerStock> findDistance(String clientSelection) throws IOException{
		...
		ClassLoader classLoader = YahooFinanceCalculate.class.getClassLoader();
        			 ...
        			 for (Entry<String, Double> entry : sorted_vectorMap.entrySet()) {
        				 ...
        				 similarityMap.get(customerName).put(entry.getKey(),similarity);
           		    	if(count<5){
           		    		...
           		    		Double calStockBid=customerMatrix.get(0).get(recStockName);String bid=df.format(calStockBid);
           		    		Double calStockChangeMA=customerMatrix.get(1).get(recStockName);String changeMA=df.format(calStockChangeMA);
           		    		Double calStockMavg=customerMatrix.get(2).get(recStockName);String Mavg=df.format(calStockMavg);
           		    		Double calStockDchange=customerMatrix.get(3).get(recStockName);String Dchange=df.format(calStockDchange);
           		    		Double calDistance= vectorMap.get(recStockName);String dis=df.format(calDistance);
           		    		//System.out.print("customerName: "+customerName+" stockName: "+recStockName+" "+"stock similarity: "+similarity+"\n");
           		    		CalStock calStock=new CalStock(recStockName,similar,customerName,bid,changeMA,Mavg,Dchange,dis);
           		    		
           		    		List<CalStock> tmp= cusStock.getCalculationBlock();
           		    		tmp.add(calStock);
           		    		count++;
           		    	}
           		     }
